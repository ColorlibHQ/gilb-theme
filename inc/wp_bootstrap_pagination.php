...
function gilb_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'gilb' ),
        'next_string'     => esc_html__( '&raquo;', 'gilb' ),
        'before_output'   => '<div class="gilb-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'gilb_pagination_defaults', $defaults )
    );
