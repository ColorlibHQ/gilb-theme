...
final class Gilb {
	...
	private function setup() {
		...
		$enqueu          = new gilb_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->gilb_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'gilb_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'gilb', GILB_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'gilb_site_logo_65x24', 65, 24, true );

		// Latest post thumbnail Widget thumbnail size
		add_image_size( 'gilb_widget_post_thumb', 70, 70, true );

		// Portfolio image sizes
		add_image_size( 'gilb_portfolio_img_360x378', 360, 378, true );
		add_image_size( 'gilb_portfolio_img_750x787', 750, 787, true );
		add_image_size( 'gilb_portfolio_img_750x378', 750, 378, true );
		add_image_size( 'gilb_portfolio_details_img_555x544', 555, 544, true );

		// Review client thumbnail size
		add_image_size( 'gilb_review_client_thumb_90x90', 90, 90, true );

		// About thumbnail size
		add_image_size( 'gilb_about_thumb_487x514', 487, 514, true );
		
		// Latest post thumbnail Widget thumbnail size
		add_image_size( 'gilb_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'gilb_single_blog_750x375', 750, 375, true );
		add_image_size( 'gilb_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'gilb' ),
			'footer-menu'  => esc_html__( 'Footer Menu', 'gilb' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = GILB_DIR_CSS_URI;
		$jsPath  = GILB_DIR_JS_URI;
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'gilb' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate gilb theme customizer
		$gilb_theme_customizer = new gilb_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'gilb_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'gilb-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'gilb_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'gilb-elementor-default-style',  GILB_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->gilb_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'gilb_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'gilb-elementor-notice', GILB_DIR_JS_URI.'gilb-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'gilb-elementor-notice',
				'gilbElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'gilb-elementor-notice-nonce' ),
				)
			);
		}
